...
int main(void){  
  ...
  while(1){
		PF0 = GPIO_PORTF_DATA_R & 0x01; //switch01
		PF4 = GPIO_PORTF_DATA_R & 0x10;	//switch02
		Led = GPIO_PORTF_DATA_R & 0x02; //led (red)
		
		//toggles LED when switch pressed
		if( PF0 == 0x00 || PF4 == 0x00) { 
			...
			if(i<50){
				...
				Data[i] = GPIO_PORTF_DATA_R&0x13; //PF1
				last = now;
				i++;
			}
		}
